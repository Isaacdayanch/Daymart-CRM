...
import { TIPOS_PROMOCION, type CambioPromocion } from "@/lib/mercadolibre-promociones";
import { deshacerPromocionMl } from "../actions";

const MODOS: Record<string, string> = { FIJO: "precio fijo", PORCENTAJE: "por %", MARGEN: "por margen" };

/** Últimas promociones aplicadas/quitadas desde el CRM, con "Quitar promoción". */
export function Bitacora({ cambios }: { cambios: CambioPromocion[] }) {
  ...
  return (
    <details className="group rounded-2xl border border-zinc-200 bg-white shadow-sm">
      <summary className="cursor-pointer select-none px-5 py-3 text-sm font-medium text-zinc-700">
        ...
        Promociones aplicadas desde el CRM ({cambios.length})
      </summary>
          ...
          <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 px-5 py-2.5 text-sm">
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-zinc-400">
                ...
                {" · "}
                {(c.accion ?? "APLICAR") === "QUITAR" ? "se quitó" : "se aplicó"} {TIPOS_PROMOCION[c.promocion_tipo ?? "PRICE_DISCOUNT"] ?? c.promocion_tipo}
                {c.fin_promocion && ` · hasta ${formatoFechaHoraMx(c.fin_promocion)}`}
            ...
            <div className="flex items-center gap-3">
              <p className="text-zinc-700">
                {(c.accion ?? "APLICAR") === "QUITAR" ? (
                  <>vuelve a {formatoPesos(c.precio_nuevo)}</>
                ) : (
                  <>
              ...
              {c.resultado !== "OK" ? (
                ...
                <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-[11px] text-zinc-500">quitada</span>
              ) : (c.accion ?? "APLICAR") === "APLICAR" ? (
                <button
                  type="button"
                  disabled={ocupado === c.id}
                  onClick={async () => {
                    if (!window.confirm(`¿Quitar la promoción de “${c.titulo ?? c.item_id}”? La publicación vuelve a su precio base.`)) return;
                    setOcupado(c.id);
                    setError(null);
                    const r = await deshacerPromocionMl(c.id);
                    setOcupado(null);
