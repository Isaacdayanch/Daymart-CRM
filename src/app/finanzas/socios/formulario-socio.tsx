...
export function FormularioSocio({ socios, cuentas }: { socios: Socio[]; cuentas: CuentaFinanciera[] }) {
  const router = useRouter();
  const [tipo, setTipo] = useState<TipoMovimientoSocio>("APORTE");
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
      <h2 className="text-sm font-semibold text-zinc-900">Registrar aporte o reparto</h2>
      <div className="mt-3 flex gap-1.5 rounded-xl bg-zinc-100 p-1">
        {(["APORTE", "REPARTO"] as const).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTipo(t)}
          ...
          >
            {t === "APORTE" ? "Aporte de capital" : "Reparto"}
          </button>
        ))}
      </div>
      <form
        ...
        action={async (formData) => {
          ...
          const resultado = await registrarMovimientoSocio(formData);
          setEnviando(false);
      ...
      >
        <div>
          <label className="block text-xs font-medium text-zinc-500">Socio</label>
          <div className="mt-1">
            <Selector
              name="socio_id"
              defaultValue={socios[0]?.id}
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-500">
            {tipo === "APORTE" ? "Cuenta destino" : "Cuenta de origen"}
          </label>
          <div className="mt-1">
            <Selector
              name="cuenta_id"
        ...
        <div className="flex justify-end sm:col-span-2">
          <button
            type="submit"
            disabled={enviando || socios.length === 0}
